// ═══════════════════════════════════════════════════════════
// ODARA PQCDSM v13.1 — Shared Module
// Mudanças v13.1:
// - agruparDosadosDoDia reconhece d.sku = 'PR01001'/'PR01002'/'PR01037' como intermediário direto
// - novo: sobrepesoIsoladoLinhaDia(entries, data, linha, pesagensTodas)
// - novo: sobrepesoIsoladoSku(dataEmb, linha, sku, gram, alfEmb, dataDose, pesagensTodas)
// - novo: getPerdasEspecificasPorSku(entries, tipo)
// ═══════════════════════════════════════════════════════════
...
function isIntermediarioCode(cod){return !!INTERMEDIARIOS[cod];}
...
const fmt2=(n)=>(n==null||isNaN(n))?'0,00':(+n).toFixed(2).replace('.',',');
const fmt2en=(n)=>(n==null||isNaN(n))?'0.00':(+n).toFixed(2);
const r2=(n)=>(n==null||isNaN(n))?0:+(+n).toFixed(2);
...
// ── 1) AGRUPAMENTO DE DOSADOS POR INTERMEDIÁRIO (v13.1 corrigido) ──
// CORREÇÃO v13.1: reconhece d.sku = 'PR01001'/'PR01002'/'PR01037' como intermediário direto
function agruparDosadosDoDia(entries){
  ...
  entries.forEach(e=>{
    (e.dosagens||[]).forEach(d=>{
      ...
      let chave,info,isFinal=false;
      // Caso 1: d.sku é código intermediário direto (PR01001/PR01002/PR01037)
      if(INTERMEDIARIOS[d.sku]){
        chave=d.sku;
        info=INTERMEDIARIOS[d.sku];
      }
      // Caso 2: d.sku é SKU final com intermediário associado (compat legado)
      else if(getIntermediario(d.sku)){
        const inter=getIntermediario(d.sku);
        chave=inter;
        info=INTERMEDIARIOS[inter];
      }
      // Caso 3: SKU final sem intermediário (crocante/peanut)
      else{
        chave='SKU_'+d.sku;
        isFinal=true;
      }
      ...
      if(!isFinal){
        if(!grupos[chave])grupos[chave]={cod:chave,desc:info.desc,kgTotal:0,alfTotal:0,skus:[],linhas:new Set(),isFinal:false,categoria:info.categoria};
      }else{
      ...
      if(d.sku!==chave&&!grupos[chave].skus.find(s=>s.sku===d.sku))grupos[chave].skus.push({sku:d.sku,desc:d.desc||''});
    });
  });
...
// Soma perdas específicas de UM tipo agrupadas por SKU → {sku: kgTotal}
function getPerdasEspecificasPorSku(entries,tipo){
  const out={};
  entries.forEach(e=>{
    (e.perdasEspecificas||[]).forEach(pe=>{
      if(pe.tipo!==tipo)return;
      const sku=pe.sku||'';const kg=+pe.kg||0;
      if(!sku||kg<=0)return;
      out[sku]=r2((out[sku]||0)+kg);
    });
  });
  return out;
}
...
function sobrepesoIsolado(pesagensFinal,pesagensDose,gramFinal,gramDose,alfEmbalados){
  ...
  const gramDoseRef=gramDose||(gramFinal*0.78);
  ...
  return{
    ...
    completo:wF.length>0&&wD.length>0
  };
}

// Para UM SKU embalado em UM dia em UMA linha
function sobrepesoIsoladoSku(dataEmb,linha,sku,gram,alfEmbalados,dataDose,pesagensTodas){
  const pFinal=(pesagensTodas||[]).filter(p=>p.linha===linha&&p.data===dataEmb&&p.sku===sku&&p.ponto==='final');
  let pDose=[];
  if(dataDose){
    pDose=(pesagensTodas||[]).filter(p=>p.linha===linha&&p.data===dataDose&&p.ponto==='combinacao');
  }
  return sobrepesoIsolado(pFinal,pDose,gram,null,alfEmbalados);
}

// Para uma LINHA inteira no dia (varre todas as embalagens)
function sobrepesoIsoladoLinhaDia(entries,dataEmb,linha,pesagensTodas){
  let totFinalKg=0,totDoseKg=0,totCobKg=0;
  let cnt=0,cntCompleto=0;
  const detalhes=[];
  entries.filter(e=>e.linha===linha&&e.data===dataEmb).forEach(e=>{
    (e.embalagens||[]).forEach(em=>{
      const real=+em.real||0;
      if(!real)return;
      const r=sobrepesoIsoladoSku(dataEmb,linha,em.sku,+em.gram||0,real,em.dataDosagem||null,pesagensTodas);
      totFinalKg+=r.sobrFinalKg;
      totDoseKg+=r.sobrDoseKg;
      totCobKg+=r.sobrCoberturaKg;
      cnt++;if(r.completo)cntCompleto++;
      detalhes.push({sku:em.sku,desc:em.desc,alf:real,dataDose:em.dataDosagem||null,...r});
    });
  });
  return{
    sobrFinalKg:r2(totFinalKg),
    sobrDoseKg:r2(totDoseKg),
    sobrCoberturaKg:r2(totCobKg),
    cnt,cntCompleto,
    completo:cnt>0&&cntCompleto===cnt,
    detalhes
  };
}
...
function perdaTotalReal(entries,sobrepesoKg){
  ...
  let espTot=0;
  entries.forEach(e=>{(e.perdasEspecificas||[]).forEach(pe=>{espTot+=(+pe.kg||0);});});
  return{A:r2(A),B:r2(B),C:r2(C),especificas:r2(espTot),total:r2(A+B+C+espTot)};
}
